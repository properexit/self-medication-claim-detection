"""
Experiment: sentence level claim type classification.

Idea:
Maybe explicitness shows up at the sentence level rather than
being uniform across the whole post.

In this setup, each sentence inherits the document label.
This is noisy, so this model is exploratory and not used
...
from src.datasets import SentenceClaimTypeDataset
...
def train():
    ...
    # Sentence level projection makes imbalance worse
    ...
    for epoch in range(EPOCHS):
        ...
        if val_f1 > best_val_f1:
            ...
            torch.save(
                model.state_dict(),
                os.path.join(SAVE_DIR, "bert_mini_claim_type_sentence_best.pt")
            )
        else:
            epochs_no_improve += 1
